...
#include </usr/local/include/opencv2/core/persistence.hpp>//add
...
// cv::Mat System::TrackRGBDyolo(const cv::Mat &im, const cv::Mat &depthmap, cv::Mat &mask,
...
// 
// 
//     cv::Mat Tcw = mpTracker->GrabImageRGBDyolo(im,depthmap,mask,timestamp,imRGBOut,imDOut,maskOut,filename);
// 
//     unique_lock<mutex> lock2(mMutexState);
//     mTrackingState = mpTracker->mState;
//     mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
			      ...
			      const double &timestamp, string filename)
{
    ...
    cv::Mat Tcw = mpTracker->GrabImageRGBDyolo(im,depthmap,mask,timestamp,filename);
...
cv::Mat System::TrackMonocularyolo(const cv::Mat &im, const cv::Mat &mask, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if(mSensor!=MONOCULAR && mSensor!=IMU_MONOCULAR)
    {
        cerr << "ERROR: you called TrackMonocular but input sensor was not set to Monocular nor Monocular-Inertial." << endl;
    ...
    cv::Mat Tcw = mpTracker->GrabImageMonocularyolo(im,mask,timestamp,filename);
