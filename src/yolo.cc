...
#include "yolo.h"
#include <vector>
#include <opencv2/dnn/shape_utils.hpp> //add
...
yolov3Segment::yolov3Segment() {
    ...
    String modelConfiguration = "src/yolo/yolov3.cfg";//tum
...
//     blobFromImage(image, blob, 1/255.0, cvSize(this->inpWidth, this->inpHeight), Scalar(0,0,0), true, false);
    blobFromImage(image, blob, 1/255.0, cvSize(160, 160), Scalar(0,0,0), true, false);//cvSize的数据要和cfg里的数据匹配
    //Sets the input to the network
    this->net.setInput(blob);
    // Runs the forward pass to get output of the output layers
    vector<Mat> outs;
    this->net.forward(outs, this->getOutputsNames(this->net));//no bug here
    
    int dilation_size = 15;
    cv::Mat kernel = getStructuringElement(cv::MORPH_ELLIPSE,
                                        cv::Size( 2*dilation_size + 1, 2*dilation_size+1 ),
                                        cv::Point( dilation_size, dilation_size ) );
    //int h = inpHeight;//change start
    //int w = inpWidth;
    //cv::Mat mask = cv::Mat::ones(h,w,CV_8U);//change end
    
    cv::Mat mask = cv::Mat::ones(480,640,CV_8U);//tum
//     cv::Mat mask = cv::Mat::ones(370,1226,CV_8U);//kitti
//     cv::Mat mask = cv::Mat::ones(375,1242,CV_8U);//kitti03
//     cv::Mat mask = cv::Mat::ones(376,1241,CV_8U);//kitti00-02
...
cv::Mat yolov3Segment::postprocess(Mat& frame, const vector<Mat>& outs)
{
    ...
    //cv::Mat mask = cv::Mat::zeros(height, width,CV_8U);//change
    cv::Mat mask = cv::Mat::zeros(480,640,CV_8U);//tum
//     cv::Mat mask = cv::Mat::zeros(370,1226,CV_8U);//kitti
//     cv::Mat mask = cv::Mat::zeros(375,1242,CV_8U);//kitti03
//     cv::Mat mask = cv::Mat::zeros(376,1241,CV_8U);//kitti00-02
